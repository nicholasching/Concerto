...
`benchmark.py` runs the real pipeline, writes validated `result.json`, records progress and outputs `metrics.json`. It checks canonical positions/columns against independent truth with a 0.015 distance tolerance, reports statuses, maximum error, wall time, worker time and input hashes/tool versions. Choose `--workers 1` for a sequential baseline or `--workers 3` (default) for concurrent cameras. `--cpu-budget` caps analysis across cameras; for three inputs, budget 3 reproduces one analysis worker per camera and larger values add frame workers. Use a fresh output directory for each run. Timings exclude generation, upload and Python import/startup, include result writing, and disable debug artifacts.

`compare_performance.py` compares CPU budgets sequentially on the same original or synthetic manifest and asserts exact result equality excluding processing time. It accepts `--evidence physical|synthetic`, `--cpu-budgets 1 4 8`, optional `--reference-result` and `--debug`, and a new `--output-dir`. See the [CPU configuration and comparison guide](../../docs/otc-performance.md); never run competing heavy jobs during a comparison.
