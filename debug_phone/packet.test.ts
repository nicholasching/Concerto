...
import { calibrationPacket as canonicalPacket } from "../packages/contracts/src/otc";
import { DIAGNOSTIC_PALETTE, PACKET_SYMBOLS, SYMBOL_MS, calibrationPacket, colorAt, encodeDeviceId } from "./public/packet.js";
...
test("standalone debug phone packet exactly matches the canonical OTC contract", () => {
  for (const deviceId of [0, 1, 7, 1024, 2047]) {
    for (const runTag of [0, 37, 255]) {
      ...
      expect(packet).toEqual(canonicalPacket(deviceId, runTag, "otc-v1"));
