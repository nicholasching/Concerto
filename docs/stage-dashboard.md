...
New calibrations use OTC v2: full red `#FF0000` and blue `#0066FF`, 47 symbols at 250 ms (11.75 seconds), and no optical run tag. Select the correct recording for the run. Refresh `/admin` after updating the app and prepare a new run to use v2. An already prepared or recorded run retains its original colors; older orange/blue clips can still be processed with their original run metadata.
