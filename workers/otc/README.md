...
Current decoder **v1.7** supports new full-red `#FF0000` / blue `#0066FF` runs and original amber `#FFB000` / blue recordings. The manifest selects the first-color pilot/phase check; measured pilots retain the existing color tolerances. The older checkpoints below describe historical investigations. See the [red/blue handoff](../../.devcontext/teams/otc-localization/journal/20260920-red-blue-palette.md).
...
- Default `--workers 3` processes up to three cameras concurrently, capped by the shared CPU budget. Each camera receives a share of that budget for independent frame detection; OpenCV stays single-threaded per detector and each codec uses two AUTO frame/slice threads. Tracking consumes results in PTS order. The parent validates hashes first, emits progress, merges results in manifest order and writes the final map. Full RGB frames use bounded shared storage; only compact detections/metadata cross pipes and each camera owns its debug files.
- `--workers 1` retains a serial reference for benchmarks and diagnosis unless `--cpu-budget` explicitly requests parallel frames. `OTC_CPU_BUDGET=auto` (default) honors CPU affinity/container quota; a positive integer or `--cpu-budget` imposes a smaller ceiling across all cameras. Direct Python callers using spawn must call from a script protected by `if __name__ == "__main__":`; use the CLI in the backend. Failure or consumer cancellation closes the frame pool and camera siblings. Forced backend cancellation must terminate the entire process tree, not just its parent.
