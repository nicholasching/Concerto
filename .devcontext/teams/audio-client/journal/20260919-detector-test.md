...
Latest diagnostic-palette change: retain that exact 55-slot packet timing and all blue/neutral behavior, but render the zero/yellow slots as red (`#FF0000`) for the test bench. The camera monitor uses the matching red-plus-blue rule. This is a bench-only optical experiment; the production calibration palette remains unchanged.
