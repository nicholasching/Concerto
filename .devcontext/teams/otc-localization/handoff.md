...
Latest performance work: CPU-budgeted frame workers are added in `frame_worker.py` / `resources.py`; `camera_worker.py` handles bounded camera scheduling. The only `tracking.py` changes are the ordered scan adapter and moving the same distance rejection ahead of unused footprint calculations. Teammates can continue detector/geometry work without adopting new thresholds or coordinate rules. Read [benchmarks and verification](journal/20260920-frame-parallelism.md) and [configuration](../../../docs/otc-performance.md). New jobs use the updated worker directly; no backend restart is required for source changes. Environment changes require restarting the service.

Latest: decoder **otc-v1.7**, user-requested `red-blue-v1` for new runs, with legacy amber/blue compatibility. Read [palette implementation and verification](journal/20260920-red-blue-palette.md). New worker jobs load the source directly; refresh the admin page and create a new calibration to emit red. Existing run colors remain frozen.

Latest mapping verification (2026-09-19): decoder source remains otc-v1.5. Three additional geometry cases prove independent stage-facing transforms with one, two or three camera views; the full OTC gate now passes **106 Python tests**. The admin's explicit full-frame preset reprocessed four actual decoded IDs into approximate coordinates and committed map revision 10. This validates the integration, not measured seat depth; the raised-phone limitation is documented in the [mapping journal](../integration/journal/20260919-seat-map-selection.md). The prior 103-test checkpoint below predates these geometry cases.

Current integrated-main working-tree update: **otc-v1.5**, **103 Python tests** and shared gate passing. Read the [preamble/tracking investigation](journal/20260919-preamble-tracking.md): complete amber/blue headers gate published tracks; compact blue cores, exposure-band footprint recovery and robust motion prevent observed track losses. Fresh original-file checks recover **15/16/17 (3/3)** and retain **9/11 (2/2)**, with zero corrected/erased bits. Wrong-tag/membership, duplicate and crossing rejection remain intact. These single-camera results have column evidence only; live run/map state was not changed. Click Retry processing for a fresh result with the editable worker; no backend restart is needed. Check Git before assuming these edits are committed/pushed. The earlier v1.2 branch evidence below is historical, including its scale measurements.
